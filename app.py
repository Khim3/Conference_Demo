...
st.set_page_config(
    page_title="IUFlowGen Demo",  # Custom title shown in browser tab        # Custom favicon (emoji or URL)
...
def main():
    sidebar = st.sidebar
    st.title('IUFlowGen Demo')
    st.sidebar.title(':gear: Menu Settings')
    uploaded_file = sidebar.file_uploader("Upload a PDF file", type=["pdf"])
    ...
    if uploaded_file:    
        ...
        if view_chart and "dot_code" in st.session_state:
            ...
            with open("full_graph.txt", "a") as f:
                f.write(full_code)
                f.write("\n------------------------\n")
            # Save the short dot code to a text file
            with open("short_graph.txt", "a") as f:
                f.write(dot)
                f.write("\n------------------------\n")
